...
0. **Journal + AI coach.** J0 + J1 code done (2026-09-20). Remaining J1: open a *new* Claude Code session in this repo so `.mcp.json` is loaded, ask "what's my win rate on 4H breaks?" and confirm it calls `journal_stats`. Then **J2** per [`JOURNAL_AI_IMPLEMENTATION_PLAN.md`](JOURNAL_AI_IMPLEMENTATION_PLAN.md) §5: `watcher.py` → `journal.db.insert_signal`, `replay.py --to-journal`, import `signals_sol_1d_binance.csv`, `analytics.signal_history()` footer in `notify/base.format_message`.
