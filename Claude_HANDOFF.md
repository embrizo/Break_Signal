...
- **Journal rules (Session 5).** `journal/analytics.py` is the only place R / PnL / stats are computed; `db.close_trade` calls it and caches `r_multiple`/`pnl_amount` on the row. Missing user input stays NULL — never inferred. `journal` imports `core` (for `Signal`); `core` and `notify` must never import `journal` (`watcher` and `__main__` may — they import it lazily / at the top respectively). Test fixture in `tests/test_analytics.py` has hand-computed golden numbers — if it fails, analytics changed, not the data.
...
0. **Journal + AI coach.** J0–J2 done (2026-09-20). Remaining J1 check: open a *new* Claude Code session in this repo so `.mcp.json` is loaded, ask "what's my win rate on 4H breaks?" and confirm it calls `journal_stats`. Then **J3** per [`JOURNAL_AI_IMPLEMENTATION_PLAN.md`](JOURNAL_AI_IMPLEMENTATION_PLAN.md) §5: `notify/telegram_bot.py` (getUpdates long-poll, chat_id allowlist, `/trade /close /skip /event /tag /list /show /stats /ask /review`), `journal/coach.py` + `prompts.py` (Anthropic SDK tool runner, read-only tools, `claude-opus-5`), `ai:` + `telegram_bot:` config blocks. Needs `pip install anthropic` (not installed here) and API key.
